...
def coh_tmm_fast(pol, n_list, d_list, th_0, lam_vac):
    ...
    * pol, n_list, d_list, th_0, lam_vac--same as input
    ...
    return {'r': r, 't': t, 'R': R, 'T': T, 'th_list': th_list,
            'pol': pol, 'n_list': n_list, 'd_list': d_list, 'th_0': th_0,
            'lam_vac':lam_vac}
...
def coh_tmm_reverse_fast(pol, n_list, d_list, th_0, lam_vac):
    """
    Reverses the order of the stack then runs coh_tmm.
    """
    th_f = snell(n_list[0], n_list[-1], th_0)
    return coh_tmm_fast(pol, n_list[::-1], d_list[::-1], th_f, lam_vac)
...
def inc_tmm_fast_reverse(pol, n_list, d_list, c_list, th_0, lam_vac):
    """
    Reverses the order of the stack then runs coh_tmm.
    """
    th_f = snell(n_list[0], n_list[-1], th_0)
    return inc_tmm_fast(pol, n_list[::-1], d_list[::-1], c_list[::-1], th_f, lam_vac)

def inc_tmm_fast(pol, n_list, d_list, c_list, th_0, lam_vac):
    """
    Incoherent, or partly-incoherent-partly-coherent, transfer matrix method.
    ...
    * Plus, all the outputs of inc_group_layers
    ...
    coh_tmm_bdata_list = []
    for i in range(num_stacks):
        coh_tmm_data_list.append(coh_tmm_fast(pol, stack_n_list[i],
                                         stack_d_list[i],
                                         th_list[all_from_stack[i][0]],
                                         lam_vac))
        coh_tmm_bdata_list.append(coh_tmm_reverse_fast(pol, stack_n_list[i],
                                                  stack_d_list[i],
                                                 th_list[all_from_stack[i][0]],
                                                  lam_vac))
